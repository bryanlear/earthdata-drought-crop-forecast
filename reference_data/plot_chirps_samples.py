...
tifs = sorted(glob(str(DATA_DIR / 'chirps-v2.0.*.tif')))

by_year = defaultdict(list)
for f in tifs:
    m = re.search(r'chirps-v2\.0\.(\d{4})\.(\d{2})\.tif', f)
    if m:
        by_year[int(m.group(1))].append((int(m.group(2)), f))

years = random.sample(sorted(by_year.keys()), k=min(3, len(by_year)))
years.sort()

fig, axes = plt.subplots(1, 3, figsize=(18, 7))

for ax, year in zip(axes, years):
    month, fpath = random.choice(by_year[year])
